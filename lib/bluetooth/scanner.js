...
function startScan(option) {
  ...
  stopBluetoothDevicesDiscovery()
    ...
    .then((res) => {
      console.log('promise step 5:', res);

      if (scanOption.scanTime) {
        console.log('启动定时器 ' + scanOption.scanTime + 's')
        timeoutHandler = setTimeout(scanTimeout, scanOption.scanTime * 1000);
      } else {
        console.log('启动定时器 ' + MAX_SCAN_TIMEOUT + 's')
        timeoutHandler = setTimeout(scanTimeout, MAX_SCAN_TIMEOUT * 1000);
      }

      if(scanOption.started){
        scanOption.started({msg:'启动蓝牙扫描成功'});
      }
